...
export async function createResultCard(result,language='zh'){
  ...
  context.fillStyle='#f2fbf8';context.font=`700 ${language==='en'?44:58}px "ZCOOL QingKe HuangYou", sans-serif`;context.fillText(language==='en'?'OFFICE SURVIVAL TEST':'职场异变图鉴',65,98);
  context.font='700 24px sans-serif';context.fillText(language==='en'?'Answer honestly. HR is not here.':'本测试不改善命运，只负责命名。',65,132);
  ...
  context.fillStyle='#0d355d';context.font='700 34px sans-serif';context.fillText(language==='en'?'YOUR WORKPLACE TYPE':'异常员工档案',555,300);
